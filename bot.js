...
// In-memory key store (переживает рестарты — потом можно SQLite)
...
bot.callbackQuery(/buy_(.+)/, async (ctx) => {
  ...
  if (!plan) return;
  const kb = new InlineKeyboard()
    .text('⭐ Telegram Stars', `pay_stars_${ctx.match[0]}`).row()
    .text('💎 CryptoBot (USDT)', `pay_crypto_${ctx.match[0]}`);
  await ctx.editMessageText(`Plan: ${plan.label}\nPrice: ${plan.usdt} USDT / ${plan.stars} ⭐\n\nChoose payment:`, { reply_markup: kb });
  await ctx.answerCallbackQuery();
});

bot.callbackQuery(/pay_stars_(.+)/, async (ctx) => {
  if (!PROVIDER_TOKEN) return ctx.editMessageText('Stars not configured.', { reply_markup: new InlineKeyboard().text('← Back', 'back') });
...
bot.callbackQuery(/pay_crypto_(.+)/, async (ctx) => {
  if (!CRYPTOBOT_TOKEN) return ctx.editMessageText('CryptoBot not configured.', { reply_markup: new InlineKeyboard().text('← Back', 'back') });
  const plan = PLANS[ctx.match[1]];
  if (!plan) return;
  try {
  ...
  } catch { await ctx.editMessageText('Payment error.'); }
  await ctx.answerCallbackQuery();
});
...
bot.callbackQuery('back', async (ctx) => {
  const kb = new InlineKeyboard()
    ...
    .text('3 Months — 25 USDT / 70 ⭐', 'buy_3months').row()
    .text('Lifetime — 50 USDT / 150 ⭐', 'buy_lifetime');
  await ctx.editMessageText('Choose:', { reply_markup: kb });
  await ctx.answerCallbackQuery();
});
